...
When users restore a previous workflow version, do they continue working on the workflow (run extraction, edit, publish) or do they abandon the workflow shortly after? Understanding post-restore behavior tells us whether version history is a recovery tool that keeps users engaged or merely a last action before giving up.
...
**Population:** All users who performed at least one restore action during the beta period (2026-03-03 to 2026-03-11). N = 214 users across 187 workflows.
...
- A new automation run on the same workflow
- A manual field/routing edit on the same workflow
- A publish attempt on the same workflow

**Definition of "abandon":** No further activity on the workflow within 30 minutes of the restore, and no return to the workflow within 24 hours.

**Data sources:**
- `analytics.example_product.workflow_versions` (restore events)
- `analytics.example_product.workflow_automation_runs` (post-restore automation runs)
- Segment session events (edits, publishes, navigation)
...
post_restore_activity AS (
    SELECT
        ...
        r.user_id,
        r.restore_ts,
        MIN(g.created_at) AS first_automation_run_after_restore
    FROM restores r
    LEFT JOIN analytics.example_product.workflow_automation_runs g
        ON r.workflow_id = g.workflow_id
...
SELECT
    ...
    COUNT(first_automation_run_after_restore) AS continued_working,
    COUNT(*) - COUNT(first_automation_run_after_restore) AS did_not_continue,
    ROUND(COUNT(first_automation_run_after_restore) * 100.0 / COUNT(*), 1) AS continue_rate_pct
...
| Unique workflows with restores | 187 |
...
**Key finding: 72% of users who restore a version continue working on the workflow within 30 minutes.** This is a strong positive signal. Version history is functioning as a recovery tool that keeps users engaged, not as a last-resort action before abandonment.

### Breakdown by action after restore

| Post-restore action (within 30 min) | % of restores |
|--------------------------------------|---------------|
| New automation run | 58% |
| Manual field/routing edit | 9% |
| Publish attempt | 5% |
| No activity (within 30 min) | 28% |

The dominant pattern is: restore, then immediately re-run extraction with a different instruction. This confirms the hypothesis that users restore to get back to a good state and then try a different approach.
...
2. **Optimize the post-restore experience.** Since 58% of users re-run extraction immediately after restoring, consider pre-populating the instruction input with a suggestion like "Try a different approach" or showing the previous instruction that led to the version they just rolled back from.
