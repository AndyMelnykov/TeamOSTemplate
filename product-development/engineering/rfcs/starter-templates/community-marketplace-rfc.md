...
The Community Marketplace is a platform feature that enables example_product users to publish their workflows as reusable templates and discover templates created by other users. The marketplace surfaces ratings and usage statistics to help users find high-quality starting points for their own contract, invoice, and intake-form automation. This creates a user-generated content flywheel where the template catalog grows organically without requiring internal effort for every new template.
...
1. **Limited catalog size.** The internal team can only produce a handful of templates per quarter, leaving gaps in coverage for niche use cases (e.g., subcontractor lien waivers, nonprofit donor intake, fitness-studio waiver forms).
2. **Users want to share.** In customer calls and feature requests, users consistently ask for a way to publish their workflows as templates so others can benefit from their work. Approximately 34% of feature requests in Q1 2026 referenced template sharing or discovery.
...
- Add "Publish as Template" flow in workflow settings
...
- Implement the `POST /api/marketplace/:id/use` endpoint to fork a template into the user's workflows
...
- Build personalized recommendation engine based on user's workflow history and category preferences
...
CREATE TABLE published_templates (
    ...
    source_workflow_id VARCHAR(36) NOT NULL REFERENCES workflows(id),
    author_id       VARCHAR(36) NOT NULL REFERENCES users(id),
    title           VARCHAR(200) NOT NULL,
    description     TEXT NOT NULL,
    category        VARCHAR(50) NOT NULL CHECK (category IN (
                        'contract', 'invoice', 'intake-form', 'purchase-order', 'hr-onboarding'
                    )),
...
[
  {
    ...
    "alt": "Vendor onboarding intake form preview",
    "order": 1
  }
]
```
...
|-----------|------|---------|-------------|
| `category` | string | (all) | Filter by category: `contract`, `invoice`, `intake-form`, `purchase-order`, `hr-onboarding` |
...
```json
{
  "templates": [
    {
      "id": "abc-123",
      "title": "Vendor Onboarding Contract Pack",
      "description": "Intake form, W-9 extraction, NDA routing, and e-signature for new vendor onboarding.",
      "category": "contract",
      "author": {
        "id": "user-456",
        "username": "janedoe",
      ...
      "preview_images": [
        { "url": "https://cdn.example_productlabs.dev/templates/abc-123/preview-1.png", "alt": "Vendor onboarding overview", "order": 1 }
      ],
      "fork_count": 142,
      "avg_rating": 4.6,
...
|--------|-----------|------|
| 400 | Invalid `category` or `sort` value | `{ "error": "invalid_parameter", "message": "Invalid category. Must be one of: contract, invoice, intake-form, purchase-order, hr-onboarding" }` |
...
```json
{
  ...
  "preview_images": [
    { "url": "https://cdn.example_productlabs.dev/templates/abc-123/preview-1.png", "alt": "Vendor onboarding overview", "order": 1 },
    { "url": "https://cdn.example_productlabs.dev/templates/abc-123/preview-2.png", "alt": "Approval routing steps", "order": 2 }
  ],
  "source_workflow_id": "wf-789",
  "fork_count": 142,
  "avg_rating": 4.6,
  "rating_count": 38,
  "status": "approved",
...
Publish a workflow as a community template. Requires authentication.
...
| 403 | User does not own the source workflow | `{ "error": "forbidden", "message": "You can only publish templates from your own workflows" }` |
| 409 | This workflow is already published as a template | `{ "error": "conflict", "message": "This workflow has already been published as a template" }` |
| 500 | Database error | `{ "error": "internal_error", "message": "An unexpected error occurred" }` |
...
Fork a published template into the user's workflows. Creates an independent copy. Requires authentication.

**Success Response (201):**

```json
{
  "workflow_id": "wf-new-456",
  "message": "Template forked successfully. Opening your new workflow.",
  "redirect_url": "/workflows/wf-new-456"
}
```
...
│  ○ Contract│  └─────────┘ └─────────┘ └─────────┘      │
│  ○ Invoice │  ┌─────────┐ ┌─────────┐ ┌─────────┐      │
│  ○ Intake  │  │ Template │ │ Template │ │ Template │      │
│  ○ PO      │  │  Card 4  │ │  Card 5  │ │  Card 6  │      │
│  ○ HR      │  └─────────┘ └─────────┘ └─────────┘      │
│            │                                             │
│            │  [Load More]                                │
├────────────┴─────────────────────────────────────────────┤
...
- **Contract** - Vendor contracts, NDAs, sales agreements, lease agreements
- **Invoice** - AP/AR invoice processing, purchase reconciliation
- **Intake Form** - Patient intake, loan applications, KYC forms
- **Purchase Order** - Procurement, supplier POs, bills of lading
- **HR Onboarding** - Employee onboarding paperwork, benefits enrollment
...
- Appears as a bottom sheet/modal after the user has been working in a forked workflow for >10 minutes
...
- Triggered from workflow settings ("Publish as Template" button)
...
3. Reviewer checks for:
   ...
   - Working configuration (template can be forked and its extraction rules/routing render correctly)
...
- **Author reputation:** Accounts must have at least one published workflow and be older than 7 days to publish templates.
...
- The `source_workflow_id` is stored for provenance but the source workflow's configuration is snapshotted at publish time -- the fork operation copies from the snapshot, not the live workflow.
