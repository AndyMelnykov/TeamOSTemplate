...
This RFC proposes a Credit Usage Dashboard that gives example_product users full visibility into how they consume credits across extraction, refinement, and publish operations (credits are metered per page processed). The dashboard will display a daily breakdown by category, real-time burn rate calculations, and projections for when credits will be exhausted at current consumption rates. Users will also be able to drill into per-workflow usage and filter by date range, category, and workflow.
...
- **Surprise limit hits**: Users in the middle of processing a batch of documents are blocked without warning when credits run out. This is the number one billing-related complaint in support (averaging 47 tickets/week in Q1 2026).
- **No category visibility**: Users cannot tell whether their credits are going to extraction, refinements, or publishes. This prevents them from adjusting their workflow to stay within budget.
...
CREATE TABLE credit_transactions (
    ...
    category        VARCHAR(20) NOT NULL CHECK (category IN ('extraction', 'refinement', 'publish', 'refund', 'referral')),
    reference_id    UUID,
...
CREATE INDEX idx_credit_transactions_workflow ON credit_transactions(workflow_id);
```
...
- **`reference_id`**: Links back to the originating entity (e.g., the automation run ID, publish ID) for auditability.
- **`workflow_id`**: Nullable because some credit events (e.g., referral bonuses) are not tied to a workflow.
...
#### `GET /api/billing/usage/by-workflow`

Returns credit totals broken down by workflow for the authenticated user.
...
| `start_date` | ISO 8601 date | 30 days ago | Start of date range |
| `end_date` | ISO 8601 date | today | End of date range |
| `sort_by` | string | `total_credits` | Sort field (`total_credits`, `workflow_name`, `last_activity`) |
    ...
    "workflows": [
      {
        ...
        "workflow_name": "Vendor Onboarding Contracts",
        "extraction": 340,
        "refinement": 120,
        "publish": 55,
        "total_credits": 515,
      ...
      {
        "workflow_id": "e5f6a7b8-...",
        "workflow_name": "AP Invoice Intake",
        "extraction": 210,
        "refinement": 85,
        "publish": 30,
        "total_credits": 325,
...
- Extraction: blue (`#3B82F6`)
- Refinement: violet (`#8B5CF6`)
...
#### WorkflowBreakdownTable Component (`src/components/billing/WorkflowBreakdownTable.tsx`)

Sortable table with columns: Workflow Name, Extractions, Refinements, Publishes, Total Credits, Last Activity. Rows are clickable and navigate to the workflow detail page.
...
- **Category filter**: Multi-select dropdown with options: Extraction, Refinement, Publish. Defaults to all selected.
- **Workflow filter**: Searchable dropdown populated from the user's workflows. Defaults to "All workflows."
...
**Per-workflow totals (powers the workflow table):**

```sql
SELECT
    ct.workflow_id,
    w.name AS workflow_name,
    SUM(CASE WHEN ct.category = 'extraction' THEN ct.amount ELSE 0 END) AS extraction_credits,
    SUM(CASE WHEN ct.category = 'refinement' THEN ct.amount ELSE 0 END) AS refinement_credits,
    SUM(CASE WHEN ct.category = 'publish' THEN ct.amount ELSE 0 END) AS publish_credits,
    SUM(ct.amount) AS total_credits,
    MAX(ct.created_at) AS last_activity
FROM credit_transactions ct
JOIN workflows w ON ct.workflow_id = w.id
WHERE ct.user_id = :user_id
    AND ct.type = 'debit'
    AND ct.created_at >= :start_date
    AND ct.created_at < :end_date
    AND ct.workflow_id IS NOT NULL
GROUP BY ct.workflow_id, w.name
...
- **Sensitive data**: Credit transaction amounts are not considered PII, but `user_id` and `workflow_id` references are. API responses never include data for users other than the authenticated user.
