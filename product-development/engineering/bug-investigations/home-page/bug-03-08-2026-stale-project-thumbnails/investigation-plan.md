# Workflow card thumbnails showing outdated preview screenshots
...
## Objective
Investigate why workflow card thumbnails on the dashboard home page show outdated preview screenshots that don't reflect recent edits, even after multiple refinements.

## Background
Users reported via support (5 tickets in one week) that their workflow thumbnails on the dashboard home page look nothing like the current state of their workflow (the thumbnail is a rendered snapshot of the workflow's document/portal preview). The thumbnail generation pipeline was last updated in January 2026 when we switched screenshot providers.

## Impact Scope
- **Affected users:** ~3,400 users (22% of active base) — any user who edited a workflow after 2026-02-15
...
- **Storage:** Supabase Storage bucket `workflow-thumbnails`
...
4. The old URL pattern was `/thumbnails/{workflow_id}.png?v={updated_at}`, new pattern is `/thumbnails/{workflow_id}.png`
...
## Recommended Fix
1. Restore cache-busting query param: append `?v={workflow.updated_at}` to thumbnail URLs
2. Purge existing CDN cache for the `workflow-thumbnails` path
...
## Cross-Validation
- Manually busted cache for 3 test workflows — all showed updated thumbnails immediately
- Verified Supabase Storage has correct current screenshots for sampled workflows
- Confirmed CDN purge API works for the thumbnail path prefix

## Data Examples

| Workflow ID | Last Edit | Thumbnail Date | Stale By |
|-----------|-----------|---------------|----------|
| wf_x8a2 | 2026-03-07 | 2026-02-16 | 19 days |
| wf_k3f1 | 2026-03-05 | 2026-02-20 | 13 days |
| wf_m9c7 | 2026-03-08 | 2026-02-15 | 21 days |

## Executive Summary
Workflow thumbnails appear stale because a cache-busting query parameter was dropped during the screenshot provider migration in February. The CDN serves old cached versions indefinitely. Restore the `?v={updated_at}` param on thumbnail URLs, purge the CDN cache, and reduce cache TTL to 1 hour. No backend or database changes needed — this is a frontend URL fix plus a one-time cache purge.

## Appendix

### Query 1: Workflows with stale thumbnails
```sql
SELECT w.id, w.name, w.updated_at, wt.generated_at,
       EXTRACT(DAY FROM w.updated_at - wt.generated_at) as stale_days
FROM workflows w
JOIN workflow_thumbnails wt ON wt.workflow_id = w.id
WHERE w.updated_at > wt.generated_at
  AND w.updated_at > '2026-02-15'
