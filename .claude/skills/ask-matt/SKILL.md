...
- **A product requirement needs scoping before engineering starts** → **`/to-prd`**. It writes the stakeholder-facing PRD — Problem Statement, Business Opportunity, User Stories — and publishes it under `product/PRDs/`. It covers the same opening stages as `/to-spec` but for a product audience; merge onto the main flow at **`/to-spec`**, which takes the PRD as input and adds Implementation Decisions and Testing Decisions rather than re-deriving what the PRD already settled.
