...
- **2026-09-12** [docs] `.claude/commands/customer-call.md` Step 5c and `product-development/product/customers/CLAUDE.md` say feature requests are logged to "Linear / Jira / Asana," but `docs/agents/issue-tracker.md` and root `CLAUDE.md` establish GitHub Issues (via `gh`) as this repo's actual tracker — the two docs disagree on where a feature request ends up. Would have designed the new feature-request-intake command against the wrong tracker if not cross-checked against `docs/agents/issue-tracker.md` first. Fixed by updating both docs to point at GitHub Issues / `/feature-request-intake`. (minor, resolved)
- **2026-09-13** [tooling] The plan/CPO-check verification command `powershell -File scripts/check-references.ps1` fails with "running scripts is disabled on this system" on a stock Windows PowerShell execution policy — needed `powershell -ExecutionPolicy Bypass -File scripts/check-references.ps1` instead. Every plan/command in this repo that documents the bare form should note the `-ExecutionPolicy Bypass` fallback. (minor, unresolved)
- **2026-09-13** [docs] `product-development/product/PRDs/shared-components-prd.md` is missing the `## Sources` section required by `product-development/product/PRDs/CLAUDE.md`'s 7-section template — discovered while dry-running the Step 6 CPO check's new Review Tier item against it. Out of scope for the Review Tier work; a future pass should backfill Sources here the same way `ai-gen-v3-prd.md` was backfilled. Backfilled per the `ai-gen-v3-prd.md` pattern (integrity-check `prd-missing-sections` finding). (minor, resolved)
- **2026-09-14** [tooling] Local `main` had never been fetched/updated since 2026-07-30 while `origin/main` was already current through PR #19 (2026-09-12) — computing `git log main..HEAD` / `git diff main..HEAD` against the stale local ref produced a 404-file, 58-commit diff that looked like it included the whole repo's history, when the real diff against `origin/main` was 14 files / 11 commits. `git fetch origin main` before diffing against a named base branch would have caught this immediately. (minor, resolved — fixed locally by fetching before diffing this session; recurs for the next agent unless local `main` is kept in sync)
